<?php
    ...
    if(isset($_POST['enviar'])) {
        ...
        $sql = 'INSERT INTO usuarios(username, nome_completo, nome_exibido, entrada, cpf, rg, nascimento, nome_mae, nome_pai, endereco, num_casa, bairro, cep, telefone, email, senha)
            VALUES (:username, :nome_completo, :nome_exibido, :entrada, :cpf, :rg, :nascimento, :nome_mae, :nome_pai, :endereco, :num_casa, :bairro, :cep, :telefone, :email, :senha)';

        try {
            $create = $db -> prepare($sql);
            $create -> bindValue(':username', $username, PDO::PARAM_STR);
            $create -> bindValue(':nome_completo', $nome_completo, PDO::PARAM_STR);
            $create -> bindValue(':nome_exibido', $nome_completo, PDO::PARAM_STR);
